...
class HMM:
    ...
    # 训练开始, 其实就是3个矩阵的求解过程
    def train(self):
        if os.path.exists("three_matrix2.pkl"):  # 如果已经存在参数了 就不训练了
                self.init_matrix, self.transfer_matrix, self.emit_matrix =  pickle.load(open("three_matrix2.pkl","rb"))
                return
        ...
        pickle.dump([self.init_matrix, self.transfer_matrix, self.emit_matrix], open("three_matrix2.pkl", "wb")) # 保存参数
